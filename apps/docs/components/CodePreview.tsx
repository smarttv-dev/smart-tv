...
export default function CodePreview({ code, language = 'bash', preview }: Props): React.ReactElement {
  const copy = async () => {
    try {
  ...
  return (
    <div className="relative rounded-lg border border-gray-200 dark:border-gray-800 overflow-hidden bg-transparent">
      {preview ? <div className="p-3 bg-gray-50 dark:bg-gray-900">{preview}</div> : null}

      <div className="p-4 bg-white dark:bg-black">
        <button onClick={copy} className="absolute right-4 top-4 z-10 px-2 py-1 text-xs rounded bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700">Copy</button>
        ...
        <pre className="whitespace-pre-wrap font-mono text-sm text-gray-900 dark:text-gray-100">
          <code>
            {lines.map((ln, idx) => {
              ...
              const parts = tokenizeJS(ln + (idx < lines.length - 1 ? '\n' : ''));
              return (
                <div key={idx} className="leading-6">
                  {parts.map((p, i) => (
                    <span key={i} className={
