...
import { getLayoutByStyle, mergeLayoutWithCustomButtons } from '../types/PlayerLayouts';
...
const Button = ({
    ...
    handleButtonAction: (action: PlayerButtonAction, config: PlayerButtonConfig) => void;
}) => {
    const { ref: buttonRef, focused: buttonFocused } = useFocusable({
        focusKey: config.focusKey || `${config.action}-${config.position}-${config.align}`,
...
const PlayerController: React.FC<PlayerControllerProps> = ({
    ...
    const finalLayout = customButtons.length > 0
    ...
    const [isTrackSelectorVisible, setIsTrackSelectorVisible] = useState(showTrackSelector);
    const [currentTrackSelectorType, setCurrentTrackSelectorType] = useState(trackSelectorType);
    const [isSettingsPanelVisible, setIsSettingsPanelVisible] = useState(false);
    const hideTimeoutRef = useRef<NodeJS.Timeout | null>(null);

    // Focus management
    const { ref, focused, hasFocusedChild, focusKey: controllerFocusKey } = useFocusable({
        focusKey,
        trackChildren: true,
    ...
    useEffect(() => {
        ...
        const shouldShow = focused || hasFocusedChild || isTrackSelectorVisible || isSettingsPanelVisible || paused;
    ...
    }, [focused, hasFocusedChild, isTrackSelectorVisible, isSettingsPanelVisible, paused, finalLayout.autoHide, finalLayout.autoHideDelay]);
    ...
    useEffect(() => {
        ...
        const handleMouseMove = () => {
            ...
            if (finalLayout.autoHide && !focused && !hasFocusedChild && !isTrackSelectorVisible && !isSettingsPanelVisible) {
                const timeout = setTimeout(() => {
                    setIsVisible(false);
                }, finalLayout.autoHideDelay || 4000);
                hideTimeoutRef.current = timeout;
    ...
    }, [finalLayout.showOnHover, finalLayout.autoHide, finalLayout.autoHideDelay, focused, hasFocusedChild, isTrackSelectorVisible, isSettingsPanelVisible]);

    // Button action handlers
    const handleButtonAction = useCallback((action: PlayerButtonAction, config: PlayerButtonConfig) => {
    ...
    const renderButtonContent = (config: PlayerButtonConfig) => {
        switch (config.action) {
            ...
            case 'playlisttoggle':
                return (
                    <PlaylistButton
                        ...
                        itemCount={playlist?.state?.rails?.reduce((total: number, rail: any) => total + rail.items.length, 0) || 0}
                        className="player-w-full player-h-full"
                    />
                );
            ...
            default:
                ...
                if (config.icon) {
                    return typeof config.icon === 'function' ? config.icon() : config.icon;
                }
    ...
    const groupedButtons = finalLayout.buttons.reduce((acc, button) => {
    ...
    const renderButtons = (position: string, align: string) => {
        ...
        return (
            <div className={cn('player-flex player-items-center player-flex-1', finalLayout.gap)}>
                {buttons.map((config, index) => {
                    ...
                    if (config.action === 'playpause' ) {
                        return (
                            <div key={index} className={cn("player-flex player-items-center player-justify-center player-w-10 player-h-10", config.className)}>
                                {content}
                            </div>
                        );
                    }
                    ...
                    // Regular button
                    return (
                        <Button key={index} config={config} handleButtonAction={handleButtonAction}>
                            {content}
                        </Button>
                    );
                })}
            ...
            <div className="player-absolute player-inset-0 player-pointer-events-none">
                ...
                {playlist && (
                    <PlaylistManager
                        ...
                        config={playlist.config}
                        callbacks={playlist.callbacks}
                        onItemChange={(item) => console.log('Changing to item:', item.title)}
                ...
                {isTrackSelectorVisible && (
                    <div className="player-absolute player-inset-0 player-bg-black player-bg-opacity-50 player-flex player-items-center player-justify-center player-pointer-events-auto">
                        <TrackSelector
                            type={currentTrackSelectorType === 'playbackRate' ? 'audio' : currentTrackSelectorType}
