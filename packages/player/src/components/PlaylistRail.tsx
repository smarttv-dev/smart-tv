...
const PlaylistRailComponent: React.FC<PlaylistRailProps> = ({
  ...
  const maxVisible =
    rail.maxVisible || (isExpanded ? rail.items.length : itemsPerRow);
    ...
    <div
      ...
      className={cn(
        ...
        (focused || hasFocusedChild) &&
          "player-ring-1 player-ring-blue-500 player-rounded-lg",
        className
      )}
    >
      {/* Rail Header */}
        ...
        {visibleItems.length > 0 ? (
          <div
            className={cn(
              ...
              config?.itemsPerRow
                ? `player-grid-cols-${Math.min(config.itemsPerRow, 6)}`
                : "player-grid-cols-1"
            )}
            style={{
              maxHeight: config?.maxRailHeight
                ? `${config.maxRailHeight}px`
                : undefined,
          ...
          <div className="player-text-center player-py-8 player-text-gray-500 dark:player-text-gray-400">
            <svg
              className="player-w-12 player-h-12 player-mx-auto player-mb-4 player-text-gray-300 dark:player-text-gray-600"
              fill="currentColor"
              viewBox="0 0 20 20"
            >
              <path
