...
import { usePlayerControls, usePlayerInstance } from "../hooks/useOptimizedHooks";
...
export const SpeedSelector = ({ focusKey, className, itemClass, focusClass, selectedClass, onSpeedSelect }: SpeedSelectorProps) => {
    ...
    const SpeedItem: React.FC<{
        ...
        focusClass?: string;
        selectedClass?: string;
    }> = ({ speed, focusKey, isSelected, onSelect, label, className, focusClass, selectedClass }) => {
        ...
        return (
            ...
            >
                ...
                {isSelected && (
                    <svg viewBox="0 0 24 24" fill="currentColor" className="player-w-5 player-h-5 player-text-blue-400">
    ...
    return (
        <FocusContext.Provider value={speedFocusKey}>
            <div ref={ref} className={cn('player-space-y-2 player-overflow-auto player-h-full', className)}>
                {speeds.map((speed) => (
                    <SpeedItem
                        key={speed}
                        speed={speed}
